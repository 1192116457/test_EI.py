...
class ekv_mos_model:
    ...
    def set_device_temperature(self, T):
        ...
        self.PHI = self.PHI * T / self.TNOM + 3.0 * constants.Vth(self.TNOM) * math.log(T / self.TNOM) - constants.si.Eg(
    ...
    def get_ismall(self, vsmall, ip_abs_err, iguess=None, debug=False):
        ...
        #                 tol=1.48e-8, maxiter=500, full_output=False)
        q = sp.special.wrightomega(vsmall + np.log(2)) / 2
        ismall = (q + 0.5)**2 - 0.25 # smarter way to compute ismall (by Alex)
        # q1 = sp.sqrt(.25 + ismall1) - .5
        # v1 = sp.log(q1) + 2 * q1
