...
- :class:`bsim3_device`, the device
- :class:`bsim3_model`, the model
...
#         ("N",ctypes.c_int),
#         ("NNZ",ctypes.c_int),
#         ("Ir",ctypes.c_int*SMB_MAX_DATA_SIZE),
#         ("Jc",ctypes.c_int*SMB_MAX_DATA_SIZE),
#         ("Pr",ctypes.c_double*SMB_MAX_DATA_SIZE),
#     ]
...
class bsim3_instances(object):
    def __init__(self, plist=None, nlist=None, 
                 pmodelname="", nmodelname="", ptr_model=None,
                 mp=0, np=0, mn=10, nn=10, max_node_num=0, temp=27.0):
        self.plist = plist
        self.nlist = nlist
        self.pmodelname = pmodelname
        self.nmodelname = nmodelname
...
class bsim3_device(dev_class):
    def __init__(self, part_id, nd, ng, ns, nb, W, L, model, M=1, N=1):
        ...
        **Parameters:**
        ...
        W : float
            element length [m]
        model : bsim3_mos_model instance
        ...
        self.dc_guess = [self.bsim3_model.para['VTH0']*0.1*self.bsim3_model.NPMOS,
                          self.bsim3_model.para['VTH0']*1.1*self.bsim3_model.NPMOS,
                          0]

        devcheck, reason = self.bsim3_model.device_check(self.device)
        if not devcheck:
            raise ValueError(reason + " out of boundaries.")
        
# #         total 10 parameters
# # 		d g s b level W L Vth0 lambda Kp
#         if self.bsim3_model.NPMOS == 1:
#             nlist = [self.n1, self.ng, self.n2, self.nb, 1, self.device.W, self.device.L]
#             nlistptr = (ctypes.c_double * len(nlist))(*nlist)        
#             nmodelname= self.bsim3_model.name
#             nmodelname = ctypes.c_char_p(nmodelname.encode('utf-8'))
#             plist = []
#             plistptr = (ctypes.c_double * len(plist))(*plist)
#             pmodelname = ctypes.c_char_p("".encode('utf-8'))            
#             mp, np, mn, nn = 0, 10, 1, len(nlist)
#         elif self.bsim3_model.NPMOS == -1:
#             nlist = []
#             nlistptr = (ctypes.c_double * len(nlist))(*nlist) 
#             nmodelname = ctypes.c_char_p("".encode('utf-8'))
#             plist = [self.n1, self.ng, self.n2, self.nb, 1, self.device.W, self.device.L]
#             plistptr = (ctypes.c_double * len(plist))(*plist)
#             pmodelname = self.bsim3_model.name
#             pmodelname = ctypes.c_char_p(pmodelname.encode('utf-8'))
#             mp, np, mn, nn = 1, len(plist), 0, 10
        
#         #Mosfet parameter check
#         max_node_num = 3
#         temp = 27.0
...
#         self.ptr_instance = bsim.LoadBSIM3Instance(plistptr, nlistptr,
#                                              pmodelname,nmodelname,
#                                              self.bsim3_model.ptr_model,
#                                              mp, np, mn, nn,
#                                              max_node_num, temp,
#                                              )  
        1;
    ...
    def get_drive_ports(self, op):
        ...
        **Returns:**
        ...
        In the case of a bsim3 device, the method returns:
    ...
    def __str__(self):
        rep = self.part_id + " %(nd)s %(ng)s %(ns)s %(nb)%s " + \
              self.bsim3_model.name + " w=" + str(self.device.W) + " l=" + \
    ...
    def _get_mos_type(self):
        ...
        """
        mtype = 'N' if self.bsim3_model.NPMOS == 1 else 'P'
        return mtype
    ...
    def istamp(self, ports_v, time=0, reduced=True):
        ...
        **Parameters:**
        ...
        """
        sw_vect, CS = self.bsim3_model.get_voltages(*ports_v)
        ids = self.bsim3_model.get_ids(self.bsim3_model, self.device, sw_vect)
    ...
    def gstamp(self, ports_v, time=0, reduced=True):
        ...
        **Returns:**
        ...
        gmd = self.bsim3_model.get_gmd(self.bsim3_model, self.device, sw_vect)
        gmg = self.bsim3_model.get_gm(self.bsim3_model, self.device, sw_vect)
        gmb = self.bsim3_model.get_gmb(self.bsim3_model, self.device, sw_vect)
        if gmd == 0:
            gmd = options.gmin*2
        if gmg == 0:
            gmg = options.gmin*2
        ...
        if CS == -1:
            stamp = self.bsim3_model.T1*stamp*self.bsim3_model.T2
    ...
    def update_status_dictionary(self, ports_v):
        ...
        if not (self.opdict['state'] == ports_v[0]).all() or 'gmd' not in self.opdict \
            ...
            self.opdict.update({'VTH':self.bsim3_model.get_VT(ports_v[0], self.device)})
            self.opdict.update({'W':self.device.W, 'L':self.device.L,
                                'ON':(vgs >= self.opdict['VTH'])})
            self.opdict.update({'beta':.5*self.bsim3_model.KP*self.device.W/self.device.L})
            self.opdict.update({'VOD':self.bsim3_model.NPMOS*(vgs - self.opdict['VTH']),
    ...
    def get_netlist_elem_line(self, nodes_dict):
        ...
        return "%s %s %s %s %s %s type=%s w=%g l=%g m=%g n=%g" % \
              ...
              nodes_dict[self.n2], nodes_dict[self.nb], self.bsim3_model.name,
              mos_type, self.device.W, self.device.L, self.device.M,
              self.device.N)


class bsim3_mos_model(object):
    def __init__(self, name=None, TYPE='n', ptr_model=None, para=None):

        self.name = "model_bsim3" if name is None else name
    ...
    def print_model(self):
        ...
        arr.append([self.name, "", "", TYPE + " MOS", "BSIM3 MODEL", "", "",
                    "", "",  "", "", ""])
