...
def transient_analysis(circ, tstart, tstep, tstop, method=options.default_tran_method, use_step_control=True, x0=None,
                       mna=None, N=None, D=None, outfile="stdout", return_req_dict=None, printvar=None, breakpoints=None, verbose=3):
    ...
    HMAX = tstep if tstep > 0 else (tstop - tstart) / 20.0
    ...
    while time < tstop:
        ...
        if options.use_sparse:
            x1, error, solved, n_iter = dc_analysis.dc_solve(
                                                     ...
                                                     Ndc=N,  Ntran=D.dot(const), circ=circ,
        ...
        if solved:
            ...
            if use_step_control:
                if x_lte_coeff is not None and pred_lte_coeff is not None and prediction is not None:
                    ...
                    for index in range(x.shape[0]):
                        if lte[index, 0] != 0:
                            new_value = 5 * ((aerror[index, 0] + rerror[index, 0]*abs(x[index, 0])) / lte[index, 0]) \
                            ** (1.0 / (df.order+1)) 
                    ...
                    if (options.transient_use_aposteriori_step_control and
                        ...
                        tstep = check_step(new_step, time - old_step, tstop, HMAX, breakpoints=breakpoints)
                        #print "Apost. (reducing) step = "+str(tstep)
                        continue
                    tstep = check_step(new_step, time, tstop, HMAX, breakpoints=breakpoints)
            ...
            x = x1
            iter_n = iter_n + 1
            total_iter = total_iter + n_iter
            sol.add_line(time, x) # add new solution to the result and print it to a file
            time_sol = timeit.time() - time_sol
            total_soltime = total_soltime + time_sol
            printing.print_info_line(("Time step %d finished. NR steps: %d. Time used: %g" % (iter_n, n_iter, time_sol), 5), verbose)
            ...
            if use_step_control:
                tstep = tstep/5.0
                tstep = check_step(tstep, time, tstop, HMAX, breakpoints=breakpoints)
...
#            sys.exit(1)
    ...
    if solved:
        ...
        printing.print_info_line(("Total linear solve: %g" % (total_iter), 3), verbose)
...
def check_step(tstep, time, tstop, HMAX, breakpoints=None):
    ...
    **Returns:**
    ...
    if tstop - time < tstep:
        ...
        bp2 = breakpoints[tdiff > 0][0] if any(tdiff > 0) else []
        bp1 = breakpoints[tdiff <= 0][-1] if any(tdiff <= 0) else []
        if bp1 and bp2:
        ...
        if bp2 - (tstep + time) < options.hmin:
