...
time_fun_specs = {'sin': { #VO VA FREQ TD THETA
    ...
    'tokens': ({
        ...
        }, 'pwl': { #pwl(x y repeat repeat_time [td])
    'tokens': ({
               'label': 'x',
               'pos': 0,
               'type': list,
...
class pulse(object):
    ...
    **Parameters:**
    ...
    def __init__(self, v1, v2, td, tr, tf, pw, per):
        self.v1 = v1
        self.v2 = v2
        if per is None: # when td is omitted in some netlists (only 6 parameters). set it to 0.0
            self.td = 0.0
            self.tr = td
            self.tf = tr
            self.pw = tf
    ...
    def __call__(self, time, dt=False):
    ...
    **Parameters:**
    ...
    def __call__(self, time, dt=False):
        ...
        if time < self.td:
            ...
            tmp = self.vo + self.va * math.exp((self.td - time)*self.theta) \
    ...
    **Example:**
    ...
    def __call__(self, time, dt=False):
        ...
        if time <= max(self.x):
            ...
            return self.y[-1] # force voltage to be the voltage of the last point-pair
