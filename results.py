...
#            csvlib.write_csv(self.filename, data[self.idx], self.variables, append=self._init_file_done)
            if self.printvar is None:
                csvlib.write_csv(self.filename, data, self.variables, append=self._init_file_done)
            else:
#                idx = [i for i, val in enumerate(self.variables) if val in self.printvar]
                csvlib.write_csv(self.filename, data[self.idx], self.variables, append=self._init_file_done)
...
class solution(object):
    ...
    **Parameters:**
    ...
    """
    def __init__(self, circ, outfile, printvar=None):
...
#                    i = circ.find_vde_index(elem.part_id)
                    i = circ.find_vde_index(elem) # passing element is faster (Alex) 
                    nv_1 = circ.get_nodes_number() - 1
...
#                    local_i_index = circ.find_vde_index(elem) # don't use element itself as we are searching for the controlling unit's vde_index
                except ValueError:
                    raise RuntimeError("Sensing voltage source %s for %s not found. BUG!" %
                                       (elem.source_id, elem.part_id))
#                local_i_index2 = circ.find_vde_index(elem.part_id)
                local_i_index2 = circ.find_vde_index(elem) # passing element is faster (Alex) 
                tot_power = tot_power - elem.alpha*x[nv_1 + local_i_index, 0]* \
                                        x[nv_1 + local_i_index2, 0]
            elif elem.is_voltage_defined:
                i_index = i_index + 1
               
        print(" get_elem_op time: {} s".format(time.time() - t1))
...
#        sys.exit()
    ...
    def gmin_check(op2, op1):
        ...
        **Returns:**
        ...
        check_failed_vars = []
        if np.linalg.norm(np.array(op1.values()) - np.array(op2.values())) > 1e-3:
            check_failed_vars = [1]
...
class tran_solution(solution, _mutable_data):
    ...
    **Parameters:**
    ...
    """
    def __init__(self, circ, tstart, tstop, op, method, outfile, printvar):
        solution.__init__(self, circ, outfile, printvar)
        self.sol_type = "TRAN"
        self.start_op = op
        ...
        self.idx = [0] # index to read the time point

        # variables names for the data to be printed to the outfile
        # use printvar if it is definied, and all variables otherwise
        if printvar is not None: 
            iv_rex = re.compile('\((.*?)\)', re.IGNORECASE | re.DOTALL)
            x_rex =  re.compile(':(.*)_', re.IGNORECASE | re.DOTALL)      
            for varname in printvar:
                if varname[0] == 'v':
                    self.variables += [varname]
                    self.units.update({varname:"V"})
                    node_name = iv_rex.search(varname).group(1)
                    self.idx += [circ.nodes_dict.get(node_name)]
                elif varname[0] == 'i':
                    self.variables += [varname]
                    self.units.update({varname:"A"})
                    vde_name = iv_rex.search(varname).group(1)
                    self.idx += [nv_1 + circ.get_elem_by_name(vde_name).vde_index + 1]
                elif varname[0] == 'n':
                    self.variables += [varname]
                    self.units.update({varname:""})
                    mem_name = x_rex.search(varname).group(1)
                    self.idx += [nv_1 + circ.ni + circ.get_elem_by_name(mem_name).mem_index + 1]
                else:
                    raise Exception('undefined print variables {}'.format(varname))
        else:
            for index in range(nv_1):
...
#                    self.errors.update({varname: error[index, 0]})
                    ...
                    self.idx += [index]
#        index = nv_1
#        nc = circ.get_current_number()
#        for elem in circ:           
#            if elem.is_voltage_defined: 
#                index += 1
#                varname = ("I%s" % (elem.part_id.upper(),)).upper()
#                if (printvar is None) or (varname in printvar):   
#                    varname = ("I(%s)" % (elem.part_id.upper(),)).upper()
#                    self.variables += [varname]
#                    self.units.update({varname:"A"})
#                    self.idx += [index] 
...
#                varname = ("X%s" % (elem.part_id.upper(),)).upper()
#                if (printvar is None) or (varname in printvar):  
#                    varname = ("X(%s)" % (elem.part_id.upper(),)).upper()
#                    self.variables += [varname]
#                    self.units.update({varname:""})
#                    self.idx += [nv_1 + nc + elem.mem_index + 1]
#                else:
#                    continue
#        
