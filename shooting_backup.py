...
def shooting_analysis(circ, period, step=None, x0=None, points=None, autonomous=False,
    ...
    **Returns:**
    ...
    #                                       tstop=1*points*step, method="GEAR2",
    ...
    if not load_xtran:
        ...
        npoints = len(xtran['t'])
        filename = 'xtran_buck_' + str(npoints) + '.mat'
    ...
    #                                                n_of_var, verbose)
    Tass_static_vector = _build_Tass_static_vector_adaptive(circ, Tf, tpoints, step, tick,
                                                   n_of_var, verbose)
    ...
    while True:
        ...
        for index in range(npoints):
            step = tpoints[index + 1] - tpoints[index] if index < npoints - 1 else step
            if index == 0:
                xn_minus_1 = x[npoints - 1]
            else:
            ...
            MAass = MAass_variable + MAass_static
            tmat = timeit.time() - tmat
            tmat_total += tmat
            
            tlu = timeit.time()
            lu_MAass = sp.sparse.linalg.splu(MAass)
            MAass_variable_vector.append(lu_MAass)
            tlu = timeit.time() - tlu
            tlu_total += tlu
        ...
        x[npoints - 1] = td * dxN + x[npoints - 1]
        ...
        dx_norm = _vector_norm_wrapper(dx, vector_norm)
        x_norm =_vector_norm_wrapper(x, vector_norm)  
        print('  PSS: iter={0},  dx_norm={1}'.format(iteration, dx_norm))                          
        if dx_norm < (min(options.ver, options.ier) * x_norm + min(options.vea, options.iea)):
        ...
        elif vector_norm(dx[npoints - 1]) is np.nan:  # needs work fixme
            raise OverflowError
            # break
        else:
            conv_counter = 0
...
def shooting_analysis_EI(circ, period, step=None, x0=None, points=None, autonomous=False,
             matrices=None, outfile='stdout', vector_norm=lambda v: max(abs(v)), printvar=None, breakpoints=None, verbose=3):
    """Performs a periodic steady state analysis based on the algorithm described in:
    ...
    if not load_xtran:
        ...
        xtran = spio.loadmat('xtran_buck_211.mat')
     # printvar=None, breakpoints=None, verbose=3
    if xtran is None:
        print("failed.")
        return None
