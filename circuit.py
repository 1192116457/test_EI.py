...
class Circuit(list):
    ...
    **Parameters:**
    ...
    def __init__(self, title, filename=None):
        ...
        self.nv = 0 # number of nodal variables
        self.ni = 0 # number of current variables (voltage defined elements)
        self.nx = 0 # number of x state variables (memristors)
        self.nidc = 0 # number of independent current sources
        self.nidv = 0 # number of independent voltage sources
        self.GTP = [] # global transition points
    ...
    def get_current_number(self):
        """Return the number of current variables in the circuit (# of voltage defined elements)"""
        cnt = 0
        for elem in self:
    ...
    def ext_node_to_int(self, ext_node):
        ...
        **Returns:**
        ...
        try:
            ...
            return -1 # when no node id is found
    ...
    def find_vde_index(self, elem_or_id, verbose=3):
        ...
        **Returns:**
        ...
        """
        # a faster implementation avoiding looping over entire circ (Alex)
        if type(elem_or_id) in py3compat.string_types:
            # we got a string corresponding to the part_id of an element
            # need to get the element back
            elem = self.get_elem_by_name(elem_or_id)
        else:
            elem = elem_or_id
        
        if hasattr(elem, 'vde_index'):
            return elem.vde_index
        else:
            raise ValueError(("find_vde_index(): element %s was not found." +\
                              " This is a bug.") % (elem.part_id,))
...
#        return vde_index
    ...
    def find_vde(self, index):
        ...
        **Returns:**
        ...
        for e in self:
            if (hasattr(self, 'vde_index') and e.vde_index == index):
                break
...
def is_elem_voltage_defined(elem):
    ...
    **Returns:**
    ...
    if (hasattr(elem, "is_voltage_defined") and elem.is_voltage_defined) \
        or isinstance(elem, devices.VSource) or isinstance(elem, devices.EVSource) or \
...
class subckt:
    ...
    **Parameters:**
    ...
    """
    def __init__(self, name, code, connected_nodes_list, params_dict=None):
        self.name = name
...
class _circuit_wrapper:
    ...
    **Parameters:**
    ...
    def __init__(self, circ, connection_nodes_dict, parameters_dict, subckt_name, subckt_label):
        self.circ = circ
#        self.prefix = subckt_label + "-" + subckt_name + "-"
        self.prefix = subckt_label + ":" # follow Xyce's style
    ...
    def parameterize(self, code):
        
        for line, line_no in code:
            line_elements = line.split(' ', 1) # separate between 1st white space
            if line_elements[0] == ".param":
                label, value_str = line_elements[1].split('=', 1) # separate between 1st equal sign
                value_str = re.findall(r'{(.*?)}', value_str)[0]
                if value_str[0:2] == 'if':
                    value_str = re.findall(r'\((.*?)\)', value_str)[0]
                    value_str = value_str.split(',')
                    if eval(value_str[0], {'__builtins__': None}, self.params_dict):
                        value = eval(value_str[1], {'__builtins__': None}, self.params_dict)
                    else:
                        value = eval(value_str[2], {'__builtins__': None}, self.params_dict)
                self.params_dict.update({label: value})  
                continue
                       
#            idx = len(line_elements[0:2])
            expr = re.findall(r'{(.*?)}', line)
            for elem in expr:
                try:
                    val = eval(elem, {}, self.params_dict)
                    line = line.replace('{' + elem + '}', str(val))
                except:
                    pass                        
                
            for param in self.params_dict:
                line = line.replace('{' + param + '}', str(self.params_dict[param]))\
                            .replace('=' + param, '=' + str(self.params_dict[param]))\
                            .replace('*' + param, '*' + str(self.params_dict[param]))\
                            .replace('/' + param, '/' + str(self.params_dict[param]))\
                            .replace('+' + param, '+' + str(self.params_dict[param]))\
                            .replace('-' + param, '-' + str(self.params_dict[param]))\
            ...
            self.code += [(line, line_no)]    
        
        
#            line_elements = line.split()
#            param_idx = [idx for idx, token in enumerate(line_elements) if token.upper() == 'PARAMS:']
