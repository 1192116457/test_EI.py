...
class Component(object):
    ...
    def __init__(self, part_id=None, n1=None, n2=None, is_nonlinear=False, is_symbolic=True, value=None, is_voltage_defined=False):
        ...
        self.is_voltage_defined = is_voltage_defined # if it is a voltage-defined element (Alex)
...
class VSource(Component):
    ...
    **Parameters:**
    ...
    def __init__(self, part_id, n1, n2, dc_value, ac_value=0):
        ...
        self._time_function = None
        self.is_voltage_defined = True
        self.index = None # index in MNA vector (needed for regularization of EI)
    ...
    def dV(self, time=None):
        ...
        **Returns:**
        ...
        if (not self.is_timedependent or\
            ...
            return self._time_function(time, derivative=True) # TODO: not implemented
...
class BISource(Component):
    ...
    **Parameters:**
    ...
    """
    #F
    def __init__(self, part_id, n1, n2, func):
        self.part_id = part_id
        self.n1 = n1
        self.n2 = n2
        self.ports = ((self.n1, self.n2),)
        x = sym.symbols('x', real=True,)
        func_sym = sym.sympify(func).subs('v', x)              
        dfunc_sym = func_sym.diff(x)
        dfunc_sym = dfunc_sym.args[1] # specific to cross_sim's BI expression. Must be revised for other functions (Alex)
        self.func = sym.lambdify(x, func_sym)
        self.dfunc = sym.lambdify(x, dfunc_sym, modules=["numpy"])
        self.is_nonlinear = True
        self.is_symbolic = True
        self.is_voltage_defined = False
#        self.i_grad = grad(self.get_i)
    ...
    def get_netlist_elem_line(self, nodes_dict):
        ...
        **Returns:**
        ...
        ntlst_line : string
            The netlist line.
        """
        return "%s %s %s %s" % (self.part_id, nodes_dict[self.n1],
                                nodes_dict[self.n2], self.func)
    ...
    def get_drive_ports(self, op):
        if not op == 0:
            raise ValueError('Nonlinear dependent source %s has no output port %d' %
                             (self.part_id, op))
        return self.ports
    ...
    def gstamp(self, ports_v, time=0, reduced=True):
        ...
        v = ports_v
        gm = self.get_gm(utilities.tuplinator(v))
        if gm == 0:
            gm = options.gmin*2
...
#        gm = self.i_grad(v)
        try:
            gm = self.dfunc(v) if v else 0.0
        except:
            print('symbolic evaluation of dfunc failed for BIsource {}'.format(self.part_id))    
#        gm = (ADI*exp(abs(V)/ADV)*sign(V)^2)/ADV
        return gm     
