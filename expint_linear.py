...
#        v1 = np.vstack((sp.zeros((mna_size, 1)), [1]))
        ...
        x1, error, solved, m1, solvar = expm_ArnoldiSAI(A, D, W, v1, dt, gamma, m_max, options.lin_GMRES_atol, kvec, linsolver=linsolver)
        tot_SAI += m1
#        print(m1)
        if not solved:
...
#            W = np.hstack((-(dT), -(Tx + T)))
        ...
        texp1 = timeit.time()
        tol_exp = options.vea
        xpre, res_exp, krylovcheck, m1_SAI, solvar = expm_ArnoldiSAI(
                A, D, W, v_rhs, dt, gamma, m_max, tol_exp, kvec, linsolver=linsolver)[0:5]
        tot_SAI += m1_SAI
...
#        
#        deltaTx = Txp - Tx
        residual1 = res_exp[idx1] #+ deltaTx[idx1] 
        ...
        Jv = sp.sparse.linalg.LinearOperator(
                (x1_size, x1_size), matvec=lambda v: jacobvec(A, D, J, M21, v, dt, kvec, linsolver=linsolver))
        res_Krylov = []
...
#            vm1_norm = np.linalg.norm(vm1)
            ...
            krylovcheck1, err1 = exp_error_check(tol, v0[:n0], beta, gamma, hj, vm1, invH, expHm1)
