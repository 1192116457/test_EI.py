...
class circdata(list):
    def __init__(self, circ=None, derivatives=None, postproc=None):
        self.circ = circ
...
def main(filename, outfile="stdout", verbose=3):
    ...
    **Returns:**
    ...
    if (os.path.isfile(filename_parsed) and options.reparse==0): # load parsed data if the netlist has been parsed. If not parse and save it 
        t1 = timeit.time()
        with open(filename_parsed, 'rb') as fp:
            circ, directives, postproc_direct = pickle.load(fp)
        printing.print_info_line(("Load parsed netlist done in %g s" % (timeit.time() - t1), 3),verbose, print_nl=False)   
    elif (os.path.isfile(filename_parsed) and options.reparse==1): # load circ from stored file, but re-parse directives and postprocess.
        with open(filename_parsed, 'rb') as fp:
            circ, directives_old, postproc_direct_old = pickle.load(fp)
        (circ, directives, postproc_direct) = netlist_parser.parse_directive_and_postprocess(
        filename, circ, read_netlist_from_stdin)  
#        sys.exit()
    else:
        (circ, directives, postproc_direct) = netlist_parser.parse_circuit(
            filename, read_netlist_from_stdin, verbose)  # main netlist parser
        with open(filename_parsed, 'wb') as fp:
            try:
                pickle.dump([circ, directives, postproc_direct], fp)
                printing.print_info_line(("Parsed netlist saved in %s" % (filename_parsed), 3), verbose)
            except:
                printing.print_info_line(("Failed, no parsed netlist saved in %s" % (filename_parsed), 3), verbose)
                pass
#        printing.print_info_line(("Parsed netlist saved in %s" % (filename_parsed), 3), verbose)
#    sys.exit()    
    printing.print_info_line(("Checking circuit for common mistakes...", 6),
                             verbose, print_nl=False)
    check, reason = utilities.check_circuit(circ) # check circuits
    if not check:
        printing.print_general_error(reason)
        sys.exit(3)
    printing.print_info_line(("done.", 6), verbose)

    if verbose > 2 or _print:
...
#    sys.exit()
    ...
    for an in netlist_parser.parse_analysis(circ, directives):
        ...
        if 'printvar' not in list(an.keys()) or not an['printvar']:
            an.update(
                {'printvar': pl['printvar'] for pl in postproc_list if pl['type'] == 'print'})    
        ...
        _handle_netlist_ics(circ, [an], ic_list=ic_list)
        if verbose >= 4:
            printing.print_info_line(("Requested an.:", 4), verbose)
            printing.print_analysis(an)
        results.update(run(circ, [an])) # start simulation
