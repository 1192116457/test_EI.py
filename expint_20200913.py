...
#        nonlocal GMRES_tol
        ...
        # v = np.ones((mna_size, 1)) # test
        # C = D.toarray() * gamma
        # Cinv = np.linalg.inv(C)
        # G = (A - D).toarray()
        # M = -Cinv.dot(G)
        # vp = Cinv.dot(J.dot(v.dot(1/dt)))
        # phiMv2 = phim(M, vp, dt, 2)
        # v = v * (1/dt)
...
#        deltaTx = Txp - Tx
        ...
        if np.max(abs(rhs1)) == 0:
            ...
            GMREStol = np.max((options.lin_GMRES_rtol*10, 
                                options.lin_GMRES_atol*10 / np.max(abs(rhs1))))
...
#
            krylovcheck, err = exp_error_check(
