...
#    print(timeit.time() - t)
    ...
    (x1, error1, solved1, n_iter1) = dc_solve(mna, N,
                                              circ, Gmin=Gmin_matrix, x0=x0, linsolver=None, verbose=verbose)
    ...
    if solved1:
        ...
        (x2, error2, solved2, n_iter2) = dc_solve(
            mna, N, circ, Gmin=None, x0=x1, linsolver=None, verbose=verbose)
    else:
        solved2 = False
...
#        mna = sp.sparse.coo_matrix(mna)
#        J = sp.sparse.lil_matrix((mna_size, mna_size))
        J = 0
#        J = sp.sparse.csr_matrix((mna_size, mna_size))
    else:
    ...
    while iteration < MAXIT:  # newton iteration counter
            ...
            if options.use_sparse:
                for elem in circ:
                    if elem.is_nonlinear:
                        _update_J_and_Tx_sparse(row, col, val, Tx, x, elem, time, mna_size1)
                J = sp.sparse.csr_matrix((val, (row, col)), shape=(mna_size, mna_size))       
            else:
                J[:, :] = 0.0
                Tx[:, 0] = 0.0
                for elem in circ:
        ...
        if options.use_sparse:
            M = sp.sparse.csc_matrix(mna + nonlinear_circuit*J)
            
#            t1 = timeit.time()
#            
#            print("splu took {} s".format(timeit.time() - t1))
            
            
            t1 = timeit.time()
            if linsolver is not None:
#                print(linsolver._is_already_factorized(M))
                dx = spsolve(M, -residuo, factorize=True, squeeze=True, solver=linsolver).reshape((-1, 1))
            else:
                lu = sp.sparse.linalg.splu(M)
                dx = lu.solve(-residuo)
            printing.print_info_line(("pardiso time: {} s".format(timeit.time() - t1), 6), verbose)
        ...
        if not nonlinear_circuit:
            converged = True
            break
        elif convergence_check(x, dx, residuo, nv - 1, ni)[0]:
            converged = True
            break
    ...
    if debug and not converged:
        ...
        converged, convergence_by_node = convergence_check(
            x, dx, residuo, nv - 1, ni, debug=True)
    else:
        convergence_by_node = []
    return (x, residuo, converged, iteration, convergence_by_node)


def _update_J_and_Tx_sparse(row, col, val, Tx, x, elem, time, mna_size1=0):
