...
#    T = np.zeros((1, 1))
    ...
    for elem in circ:
        ...
        if circuit.is_elem_voltage_defined(elem) or (hasattr(elem, 'is_memristor') and elem.is_memristor):
            v_eq = v_eq + 1
...
#                T[index] = elem.dc_guess[0]
                ...
                rowT += [index]
                colT += [0]
                valT += [elem.dc_guess[0]]
                index = index + 1
    M = sp.sparse.csr_matrix((val, (row, col)), shape=(index, nv))
    T = sp.sparse.csr_matrix((valT, (rowT, colT)), shape=(index, 1))            
    if verbose == 5:
        print("DBG: get_dc_guess(): M and T, no reduction")
        print(M)
        print(T)
#    M = utilities.remove_row_and_col(M, rrow=10 * M.shape[0], rcol=0)
    M = utilities.remove_row_and_col(M, rrow=None, rcol=0)
...
#    M1 = M.copy()
#    t = timeit.time()
#    for i in range(M.shape[0] - 1, M.shape[0]-100, -1):
##    for i in range(M.shape[0] - 1, -1, -1):    
#        for j in range(i - 1, -1, -1):
#            # print i, j, M[i, :], M[j, :]
#            dummy1 = M[i, :] - M[j, :]
#            dummy2 = M[i, :] + M[j, :]
#            if not dummy1.toarray().any() or not dummy2.toarray().any():
#                # print "REM:", M[i, :]
...
#    t = timeit.time() - t 
    ...
    y = M.dot(v)
    unique, index = np.unique(np.abs(y), return_index=True)
...
##        if not M[:, i].any():
#        if M[:, i].nnz == 0:    
#            M = utilities.remove_row_and_col(M, rrow=None, rcol=i)
#            removed_index.append(i)
#    t = timeit.time() - t
    
    kvec = np.diff(M.tocsc().indptr) != 0
    M = M[:, kvec]
    removed_index = np.nonzero(~kvec)[0]           
#    if verbose > 3:
...
#        print("T\n", T)
    ...
    if options.use_sparse:
        Rp1 = sp.sparse.linalg.lsmr(M, T.toarray(), 0.0, atol=1e-6)[0]
        Rp = np.zeros((nv - 1, 1))
        Rp[kvec] = Rp1[:, None]
    else:    
