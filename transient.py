...
#    spio.savemat('crossbar.mat',{'mna': mna, 'D': D, 'N': N})    
    ...
    if x0 is None or type(x0) == int:
        ...
        if options.testcase == '1k': # for 1k_cell case only
...
#        opsol.print_short()
    ...
    if method == EI: 
        ...
        # Dfixed = sp.sparse.spdiags(np.ones(circ.nv)*options.cmin, 0, N_mna, N_mna, format='csc')
        # D = D + Dfixed
        # D, mna = utilities.remove_vsource(D, mna, circ)
...
#    
    ...
    if method == EI:  
        ...
        if len(df.idx1) != 0:
            ...
            # df.luDs = sp.sparse.liSnalg.splu(Ds * df.gamma)
    ...
    while time < tstop - options.hmin:
        ...
        if method != EI:
            ...
            if ((tstep > 10 * df.gamma) or (tstep < 0.1 * df.gamma)) and False:
            ...
            x1, Tx1, error, residual, solved, n_iter, n_gmres, n_sai = expint.ei_solve(
                                                         A=A, mna=mna, D=D, Ndc=N, circ=circ,
                                                         Gmin=Gmin_matrix, x0=x0.copy(), Tx0=Tx0.copy(), lu=lu22,
                                                         time=(time + tstep), tstep=tstep,
                                                         locked_nodes=locked_nodes,
                                                         MAXIT=options.transient_max_nr_iter,
...
#                        lteFs = lteFs[:, None]                    
                ...
                tstep = check_step(new_step, time, tstart, tstop, HMAX, breakpoints=breakpoints)
                printing.print_info_line(("time=%g, step size = %g, lte = %g, new step = %g" % (time, old_step, lte, tstep), 3), verbose)
            else:
            ...
            if use_step_control and method == EI and ltex1 is not None:
                id_max = np.argmax(ltex1[idnz1] / tol1[idnz1])
                # err_abs.append((abs(ltex1[idnz1][id_max])))
                err_rel.append(np.linalg.norm(ltex1[idnz1]) / np.linalg.norm(x1[df.idx1][idnz1]))
...
#            sys.exit(1)
    ...
    if solved:
        ...
        printing.print_info_line(("Total time steps: %d s" % (iter_n), 3), verbose)
