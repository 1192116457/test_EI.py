...
def write_csv(filename, data, headers, append=False):
    ...
    **Parameters:**
    ...
    headers = SEPARATOR*2 + headers[0] + SEPARATOR*2 + SEPARATOR.join(headers[1:]) if not append else ""
    np.savetxt(fp, data.T, fmt='%.8e', delimiter=SEPARATOR, header=headers, comments='#')
...
def get_headers(filename):
    ...
    **Returns:**
    ...
    headers = " ".join(line.split()).split() # a safer way to get headers from the 1st line of the file (Alex)
    return headers
