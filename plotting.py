...
def _split_netlist_label(labels_string):
    ...
    p = re.compile(r'V\s*\(\s*(\S*)\s*\)', re.IGNORECASE) # find 'label' within 'V(label)', modified \w to \S by Alex
    labels_list = p.findall(labels_string)
    for i in range(len(labels_list)):
        l2 = "V" + labels_list[i]
        l1 = None
        ret_labels.append((l2, l1))
    p = re.compile(r'I\s*\(\s*(\S*)\s*\)', re.IGNORECASE) # find 'label' within 'I(label)
    labels_list = p.findall(labels_string)
    for i in range(len(labels_list)):
        l2 = "I(" + labels_list[i] + ")"
        l1 = None
